...
%     % [Result,RefereceResult] = multiclass_metrics(confMatrix)
    ...
    Result.Recall=mean(stats(:,2),"omitmissing");
    Result.Precision=mean(stats(:,1),"omitmissing");
    Result.F1_score=mean(stats(:,3),"omitmissing");
    Result.Specificity=mean(Specificity,"omitmissing");
